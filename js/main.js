...
var Spotify = {
  ...
  getAlbumArt: function (tab, callback) {
    Chrome.executeScript({
      tab: tab,
      code: 'document.querySelector(".now-playing-bar-container .cover-art-image.cover-art-image-loaded").style.backgroundImage'
    }, function (res) {
  ...
  getPlayOrPauseStatus: function (tab, callback) {
    Chrome.executeScript({
      tab: tab,
      code: 'document.querySelector(\'.now-playing-bar-container  .player-controls button[class="control-button spoticon-pause-16 control-button--circled"]\')'
    }, function (res) {
  ...
  pause: function (tab, callback) {
    Spotify.click({
      tab: tab,
      query: '.now-playing-bar-container  .player-controls button[class="control-button spoticon-pause-16 control-button--circled"]'
    }, callback)
  },
  ...
  play: function (tab, callback) {
    Spotify.click({
      tab: tab,
      query: '.now-playing-bar-container  .player-controls button[class="control-button spoticon-play-16 control-button--circled"]'
    }, callback)
  },
  ...
  previous: function (tab, callback) {
    Spotify.click({
      tab: tab,
      query: '.now-playing-bar-container .player-controls button[class="control-button spoticon-skip-back-16"]'
    }, callback)
  },
  ...
  next: function (tab, callback) {
    Spotify.click({
      tab: tab,
      query: '.now-playing-bar-container .player-controls button[class="control-button spoticon-skip-forward-16"]'
    }, callback)
  }
};
...
function fetchLyrics() {
  ...
  var artist = findEl('#current-track-artist').innerText;
  var track = findEl('#current-track-name').innerText;
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      findEl('#container-lyrics').innerText = (response.mus && response.mus[0] ? response.mus[0].text : 'Sorry, no lyric available for this song.')
    }
  };
